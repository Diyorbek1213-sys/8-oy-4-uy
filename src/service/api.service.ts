...
const BASE_URL = `${import.meta.env.VITE_PROTOCOL || process?.env.VITE_PROTOCOL
...
export const YTService = {
  ...
  getVideoComments: async (id: string): Promise<{ comments: any[] }> => {
    const { data } = await axios.get(`https://youtube-v2.p.rapidapi.com/video/comments`, {
      params: { video_id: id },
    });
    return data;
  },
